    ...
    >
      ...
      <nav class="flex-1 p-3 space-y-1.5 overflow-y-auto">
        <router-link v-for="item in navItems" :key="item.to" :to="item.to"
          ...
          exact-active-class="!bg-white/15 font-bold shadow-sm"
...
<script setup>
import { ref, computed, reactive, provide, onMounted, watch, onErrorCaptured } from 'vue'
...
// DEBUG: trace route changes inside MainLayout
watch(() => route.fullPath, (newPath, oldPath) => {
  console.log("[LAYOUT] route changed:", oldPath, "->", newPath, "name:", route.name)
})

onErrorCaptured((err, instance, info) => {
  console.error("[LAYOUT] child error:", err, info)
  return false
})
