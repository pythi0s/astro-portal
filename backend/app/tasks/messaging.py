...
def send_email_task(self, customer_id: int, template_id: int, visit_id: int | None = None):
    ...
    async def _send():
        async with async_session_factory() as session:
            ...
            template = (await session.execute(select(MessageTemplate).where(MessageTemplate.id == template_id))).scalar_one_or_none()
            if not customer or not template:
                return {"status": "error", "detail": "Customer or template not found"}
            body = template.body_text
            for key, val in {"name": customer.name, "phone": customer.phone or "", "email": customer.email or ""}.items():
                body = body.replace("{{" + key + "}}", val)
            await send_email_svc(to=customer.email, subject=template.subject or "Notification", body=body)
            return {"status": "sent"}
